...
def encode_prompt_only(prompt: str, vocab: dict[str, int]) -> list[int]: # <EOS>는 prompt 단계에 안 넣고 순수 prompt만 모델 입력 앞부분으로 넣음, control prompt를 token id로 바꿈. 
...
def load_conditional_model_and_vocab(checkpoint_path: str, device: str | None = None): # fine-tuning된 모델과 vocab 불러오기. vocab은 checkpoint에 없으면 3utr vocab에서 빌드. checkpoint에 있으면 그걸로 덮어쓰기. 
    device = device or ('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    if 'transformer.wte.weight' in state:
        cfg.vocab_size = state['transformer.wte.weight'].shape[0] #config에 있는 vocab size는 무시하고 checkpoint에 있는 wte weight의 vocab size로 설정. fine-tuning할 때 control tag 추가하면서 vocab size 늘어났을 수 있어서.
...
def sample_next_token(logits, temperature=1.0, top_k=None): # logits를 temperature로 나눔. top-k 설정되어 있으면 상위 K개 토큰만 남김. 
...
def infer_length_token_bounds(prompt: str, reference_csv: str = 'data/processed/gemorna_conditional_training_table.csv', hard_cap_tokens: int = 1024): # 제어 프롬프트에 길이 관련 태그 있으면 reference_csv에서 해당 길이 버킷의 3'UTR 시퀀스 길이 분포 보고 min, max token 수 추정. hard_cap_tokens는 모델의 최대 컨텍스트 길이 고려해서 너무 긴 시퀀스 생성 안 하도록 하는 안전장치.
...
def build_control_prompt(pgk: str | None = None, length: str | None = None, prompt: str | None = None) -> str:
    ...
    if pgk is not None:
        pgk = pgk.lower().strip()
        mapping = {'high': '<pgk_high>', 'mid': '<pgk_mid>', 'low': '<pgk_low>'}
        if pgk not in mapping:
            raise ValueError("--pgk must be one of: high, mid, low")
